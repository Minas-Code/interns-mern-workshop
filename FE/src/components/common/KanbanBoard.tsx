    ...
    onDragEnd({ active, over }) {
      ...
      if (active.data.current?.type === 'Column' && over.data.current?.type === 'Column') {
        ...
        if (over.data.current.task.columnId !== pickedUpTaskColumn.current) {
          updateTask(
            {
              status: column?.id as string,
              title: over.data.current.task.title,
              description: over.data.current.task.content,
            },
            over.data.current.task.id as string
          );
  ...
  return (
    ...
    >
      ...
      <DragOverlay>
        ...
        {activeTask && <TaskCard task={activeTask} isOverlay deleteTask={(e) => console.log(e)} />}
      </DragOverlay>
    </DndContext>
  );
